...
// use labelg to canonize some graphs
function canonize_graphs($graph_list) {
   ...
   if (is_resource($process)) {
     ...
     if($debug) { print_r(array_map("stripslashes",$graph_list)); }
     fwrite($pipes[0],"\n");
    fclose($pipes[0]);
...
// Returns a subarray of $array containing all keys with $key_signature in them and nonempty values.
...
// Returns an array where each value has been trimmed with the characters in $charlist.
// This function recursively descends into arrays.
function array_trim($array, $charlist=null){
    foreach($array as $key => $value){
...
// Extracts the text from the start of $field_name up to the first occurence of $suffix
function extract_field_name($field_name, $suffix){ 
  return substr($field_name,0,strpos($field_name,$suffix));

} 

function extract_field_operator($field_name) {
  return $_POST[extract_field_name($field_name, '-value').'-operator'];
}
...
// Overwrite the default values above
...
// Field codes to the English representation
$euphamism = array(
...
if(isset($_POST['CONDITIONS'])) {
  // We're on a multipage query and already computed all this stuff
...
// Before the visible and picture things were checkboxes, so we had them listed separately.
...
// Filter rules based on substrings of the field names.

foreach (array_filter_key($QUERY,"-value") as $field => $val) {
    $fieldname=extract_field_name($field, "-");
    // Validate our input
    switch(extract_field_operator($field)) {
        case "<=": $fieldoperator = "<="; break;
        case "=":  $fieldoperator = "="; break;
        case ">=": $fieldoperator = ">="; break;
    }

    $safe_where_text[] = "[ AND ".$fieldname." ".$fieldoperator." %F ]";
    $show_query[]="[ AND ".$euphamism[$fieldname]." ".$fieldoperator." %F]";
...
if ($debug) {echo "\nVISIBLE: "; print_r($VISIBLE_FIELDS); echo "\n";}
...
if($debug) { 
    echo 'safe_where_values: '; print_r($safe_where_values); echo "\n";
    echo 'query_string: '; print_r($query_string);
}

$safe_q = $safesql->query($query_string,$safe_where_values);

if($debug) { echo "\n safe_q: "; print_r($safe_q); }
...
if($debug) {
 echo '<pre>'; 
 echo 'safe_where_text: '; print_r($safe_where_text); 
 echo 'safe_where_values: '; print_r($safe_where_values); 
 echo 'query_string: '; print($query_string."\n\n");
 echo 'safe_q: '; print($safe_q."\n\n");
 echo 'show_query: '; print_r($show_query);
...
if (mysql_num_rows($result) > 0) { 
  ...
  while($row = mysql_fetch_array($result,MYSQL_ASSOC)) { 
    ...
    foreach ($QUERY['PICTURE'] as $pic)
      echo "<td><img src='/grout/graph-pics/".urlencode($row[$pic]).".png'></td>";
